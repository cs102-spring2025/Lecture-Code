 ...
 class LinkedList {
    private:
     ...
     void LinkedList::delete_node(int index) {
        size--;
        if (index == 0 ) { // is node to delete the first node?
            Node* to_delete = head; // create a helper node to point to where head is pointing to i.e. first node
            head = head->get_next(); // now set head to point to where first node is pointing to (i.e. second)
            delete to_delete; // delete the first node
        }
        else { // node to delete is not the first node
            Node* previous = get_node(index - 1); // get the previous node which next is pointing to the node to delete
            Node* to_delete = previous->get_next(); // set a pointer to that node
            previous->set_next(to_delete->get_next()); //now set the "next" value of previous to point to the node after the node to delete
            delete to_delete;
        }
